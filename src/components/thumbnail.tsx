...
export function Thumbnail({ fallback, isLive, src, username }: ThumbnailProps) {
  ...
  if (!src) {
    content = (
      <div className="flex h-full w-full flex-col items-center justify-center gap-y-4 rounded-md bg-[#22252e] transition-transform group-hover:-translate-y-2 group-hover:translate-x-2">
        <UserAvatar
          username={username}
          size="lg"
          showBadge
